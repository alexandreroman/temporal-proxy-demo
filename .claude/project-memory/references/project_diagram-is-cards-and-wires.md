...
description: "The component cards and the one that spans both rows, hard-coded wire geometry, and the sizing rule that the dev-server loop cannot show"
type: project
---
...
alone reaches anything past itself — Temporal upstream,
and the KMS server holding the key its payloads are
sealed with. Each component is a card with a hairline
border, and the connections are SVG wires — two
disjoint diagonals converging on the proxy, and two
horizontal hops leaving it, both crossed by the
boundary that carries the "the application's knowledge
ends here" annotation.
...
The two horizontal hops share one column that flexes
between a floor and whatever the band leaves, so the
diagram fills the content width where there is room and
its edges sit flush with the result panel's; below that
the column reaches its floor and the band scrolls.
The diagonals do **not** flex, and the reason is worth
keeping: stretching a `viewBox` on one axis either
thickens their stroke, or, with
`vector-effect: non-scaling-stroke` to prevent that,
breaks the dash — a device-space dash stops honouring
`pathLength`, so the pulse never completes its wire. A
horizontal wire has neither problem, because an x-only
stretch cannot touch a stroke width measured in y.
...
**The endpoint card is the one that spans both rows.**
Both onward wires leave that station and cross the
flexing column, which only a horizontal wire survives,
so each has to meet its own row's centre — outside the
band a card centred on the grid covers. A card spanning
both rows puts its right edge across the two of them,
and its own converging wires still land on the left
edge. Its name and caption are centred in what the
stretch leaves rather than pinned to the top.

**A caption item is truncated, never wrapped.** Each
one holds a single line and ends in an ellipsis when it
does not fit, because every card but the endpoint's
shares a fixed height that a second line box would
break. The full text stays
...
own or it clips the cards' halos — and, under the rows,
the boundary's label, which hangs in that same padding.
